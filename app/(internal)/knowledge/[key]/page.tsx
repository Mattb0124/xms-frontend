...
function PropertiesRail({
  ...
  return (
    <RailCard caption="Properties">
      ...
      />
      {article.source_ticket_id ? <p className="text-xms-label mt-3 text-body">Created from a ticket.</p> : null}
      {article.generalized_from_id ? (
        <p className="text-xms-label mt-1 text-body">Generalized from an account article.</p>
      ) : null}
      {article.retired_reason ? (
        <p className="text-xms-label mt-1 text-body">Retired: {article.retired_reason}</p>
      ) : null}
    </RailCard>
  );
}
      ...
      <RecordBar
        ...
        pill={
          <span className="flex items-center gap-2">
            ...
            <span className="text-xms-label text-body">{KIND_LABEL[article.kind]}</span>
          </span>
        }
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_300px]">
        <Panel title="Article" flush>
          ...
          <div className="p-4">
            {tab === "content" ? (
              <div className="flex flex-col gap-4">
                {editingPublished && article.draft ? (
                  <div
                    className="border-xms-line bg-xms-tint flex items-center gap-3 rounded-card border px-3 py-2 text-body"
            ...
            {tab === "history" ? (
              <ul className="divide-xms-line divide-y text-body" aria-label="Versions">
                {article.versions.map((version) => (
                  <li key={version.id} className="flex items-center gap-3 py-2">
                    ...
                    <span className="text-xms-label ml-auto text-body">{version.authored_name}</span>
                  </li>
                ))}
              </ul>
            ) : null}
            {tab === "feedback" ? (
              <dl className="grid grid-cols-[160px_1fr] gap-y-2 text-body" aria-label="Feedback counts">
