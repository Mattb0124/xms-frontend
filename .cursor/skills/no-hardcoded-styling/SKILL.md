...
3. **Runtime values that cannot be enumerated at build time**: the per-account branding accent (§3.3), an SLA burn-down bar width, a virtualised row offset. Set a CSS custom property inline and let CSS consume it, so the *rule* still lives in CSS:
...
```css
/* xms-scope.css */
.xms-burndown::after { width: var(--xms-burn-pct); background: var(--xms-accent); }
```
