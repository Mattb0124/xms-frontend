...
export function IssueDetail({ issue }: { issue: Issue }) {
  return (
    <article className="mx-auto max-w-3xl p-8 space-y-6">
      ...
      />
      <RichTextEditor defaultValue={issue.body} onBlur={(body) => updateBodyAction(issue.id, body)} />
    </article>
  );
}
```
...
color. Use semantic tokens; reserve hue variation for _content
categorization_ (chart series, status pills), not for chrome.
...
- Show the _skeleton_ of what would be there (a dotted card outline,
  a faded sample row) so the user understands the shape
- Or show the _first step_ (Linear's "Create your first issue")
...
content. Linear's issue page doesn't _show_ an issue — it IS the
issue. Notion's page doesn't _contain_ your writing — it IS your
writing. Stripe Checkout doesn't _frame_ the transaction — the
