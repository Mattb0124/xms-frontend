  ...
  return (
    <div className="flex flex-col gap-4" data-testid="run-review" data-status={run.status}>
      <div className="flex flex-wrap items-center gap-3">
        <h1 className="text-xms-ink text-lead font-semibold">Report review</h1>
      ...
      >
        <dl className="grid gap-3 text-body sm:grid-cols-2 lg:grid-cols-4">
        ...
        {run.review_note ? (
          <p className="text-xms-ink mt-3 text-body">
            <span className="text-xms-label">Cancelled because: </span>
            {run.review_note}
          </p>
        ) : null}
        {run.error ? <p className="text-xms-muted mt-3 text-body">{run.error}</p> : null}
      </Panel>
        ...
        <Panel title="Decision" caption="Already taken" subtitle="This run is no longer waiting on a reviewer.">
          {run.delivery ? (
            <>
              ...
              <p className="text-xms-label mt-2 text-body">{DELIVERY_LINK_NOTE}</p>
            </>
          ) : (
            <p className="text-xms-label text-body">Nothing was delivered.</p>
          )}
        </Panel>
      )}
...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <dt className="text-xms-label text-body">{label}</dt>
      <dd className="text-xms-ink xms-mono text-body">{value}</dd>
    </div>
  );
}
    ...
    <Panel title="Narrative" caption="The words, not the numbers" subtitle={narrativeSourceLine(run)}>
      <div className="flex flex-col gap-3">
        {NARRATIVE_SECTIONS.map(({ key, title }) => (
          <label key={key} className="flex flex-col gap-1 text-body">
            <span className="text-xms-label">{title}</span>
            <textarea
              aria-label={title}
              rows={key === "headline" ? 4 : 2}
              maxLength={6000}
              className={cn(INPUT, "h-auto py-1.5 text-body")}
              value={draft[key]}
              onChange={(event) => setDraft((current) => ({ ...current, [key]: event.target.value }))}
            />
          </label>
        ))}
        {needsRegenerate(run) ? <p className="text-xms-label text-body">{UNRENDERED_EDIT_NOTE}</p> : null}
        <div>
          <button
            type="button"
            className={SECONDARY_BUTTON}
            ...
            <dl className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6" data-testid="review-tiles">
              {section.tiles.map((tile) => (
                <div key={tile.label} className="border-xms-line flex flex-col gap-1 rounded-card border p-3">
                  ...
                  <dd className="text-xms-ink xms-mono text-lead font-semibold">{tile.value}</dd>
                </div>
              ))}
            </dl>
          ) : null}
