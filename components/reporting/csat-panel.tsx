...
function Figure({ label, value, detail }: { label: string; value: string; detail?: string }) {
  return (
    <div className="border-xms-line flex flex-col gap-1 rounded-card border px-4 py-3" aria-label={label}>
      ...
      <span className="text-xms-ink text-lead font-semibold" data-value>
        {value}
      </span>
      {detail ? <span className="text-xms-label text-body">{detail}</span> : null}
    </div>
  );
}
  ...
  return (
    <ol className="flex flex-col gap-2" aria-label="Score distribution">
      {distributionRows(summary).map((row) => (
        <li
          key={row.score}
          className="grid grid-cols-[150px_1fr_40px] items-center gap-3 text-body"
      ...
      <div className="flex flex-col gap-4" data-testid="account-csat-quarterly">
        ...
        <ol className="flex flex-col gap-2" aria-label="Averages by question">
          {questions.map((row) => (
            <li
              key={row.key}
              className="grid grid-cols-[150px_1fr_70px] items-center gap-3 text-body"
          ...
          {questions.length === 0 ? (
            <li className="text-xms-label text-body">No question has been answered yet.</li>
          ) : null}
        </ol>
        <div>
          ...
          <ol className="flex flex-col gap-2" aria-label="Quarterly trend">
            {trend.map((row) => (
              <li
                key={row.period}
                className="grid grid-cols-[110px_1fr_70px_90px] items-center gap-3 text-body"
            ...
            {trend.length === 0 ? (
              <li className="text-xms-label text-body">No quarter has been answered yet.</li>
            ) : null}
          </ol>
        </div>
      </div>
  ...
  if (!allowed) {
    return (
      <Panel title="Satisfaction" caption="Needs the tickets:view permission">
        <p className="text-xms-label text-body">You can see this account but not its satisfaction scores.</p>
      </Panel>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="account-csat">
          ...
          >
            <label className="text-xms-label flex items-center gap-1">
              ...
              <input
                type="date"
                aria-label="From"
                className={cn(INPUT, "xms-mono h-[28px] w-[140px] text-body")}
                value={range.from}
      ...
      >
        ...
        {isError ? <p className="text-xms-muted text-body">The satisfaction scores could not be loaded.</p> : null}
        {data ? (
              ...
              {data.responses.map((response) => (
                <tr key={response.id} className="border-xms-line border-b" data-response={response.id}>
                  ...
                  <td className={cn(CELL, "xms-mono")}>
                    {response.ticket_key ? (
                      <Link href={`/cases/${response.ticket_key}`} className="xms-link">
                        {response.ticket_key}
                      </Link>
                    ) : (
                  ...
                  <td className={cn(CELL, "xms-mono text-xms-label text-body")}>{formatDay(response.created_at)}</td>
                </tr>
              ))}
