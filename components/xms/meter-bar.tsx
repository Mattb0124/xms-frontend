...
export function MeterBar({ percent, pauses = [], breached, paused, label, className }: MeterBarProps) {
  const fill = clamp(percent);
  const fillColor = breached ? "var(--state-overdue-text)" : fill >= 75 ? "var(--xms-sla-warn)" : "var(--xms-accent)";
  return (
