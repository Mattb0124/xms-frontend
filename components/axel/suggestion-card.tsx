...
const SMALL_BUTTON = "h-[28px] rounded-control px-3 text-body font-medium disabled:opacity-50";
      ...
      return (
        <div className="flex flex-col gap-1">
          ...
          {payload.ci_ids?.length ? (
            <p className="xms-mono text-xms-label text-body">CI {payload.ci_ids.join(", ")}</p>
          ) : null}
          {reasons.length ? (
          ...
          <ul className="flex flex-col gap-1">
            {payload.candidates.map((candidate) => (
              <li key={candidate.ticket_id} className="flex flex-wrap items-baseline gap-2">
                <span className="xms-mono text-xms-accent text-body">{candidate.ticket_id}</span>
                <span className="xms-mono text-xms-label text-body">{formatConfidence(candidate.similarity)}</span>
    ...
    case "summarise": {
      ...
      return (
        <div className="flex flex-col gap-2">
          ...
          {sources ? (
            <p className="xms-mono text-xms-label text-body">
...
function EditForm({
  ...
  switch (suggestion.capability) {
      ...
      return (
        <div className="grid gap-2 sm:grid-cols-2">
          {(["impact", "urgency"] as const).map((field) => (
            <label key={field} className="flex flex-col gap-1 text-body">
    ...
    case "duplicate": {
      ...
      return (
        <fieldset className="flex flex-col gap-1 text-body">
...
function FeedbackRow({ suggestionId }: { suggestionId: string }) {
  ...
  if (isSuccess || rated !== null) {
    return <p className="text-xms-label text-body">Thanks, rated {rated} of 5.</p>;
  }
  return (
    <div className="flex items-center gap-1 text-body" role="group" aria-label="Rate this suggestion">
...
export function AxelSuggestionCard({
  ...
  if (withheld) {
    footer = <p className="text-xms-label text-body">{withheldLine(current.withheld_reason)}</p>;
  } else if (settled) {
    footer = (
    ...
    footer = <p className="text-xms-label text-body">You can view this suggestion but not decide it.</p>;
  } else if (mode === "reject") {
    footer = (
  ...
  return (
    <article
      className={cn("xms-ai flex flex-col gap-2 p-3 text-body", className)}
    ...
    >
      <header className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xms-label text-body">Axel</span>
        <span className="xms-mono text-xms-label ml-auto text-body" title={`Prompt ${suggestion.prompt_version}`}>
          {suggestion.agent_id}
      ...
      {suggestion.explanation ? <p className="text-xms-body text-body">{suggestion.explanation}</p> : null}
