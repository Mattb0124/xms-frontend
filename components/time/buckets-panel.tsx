...
function BucketRow({
    ...
    return (
      <li className="border-xms-line border-b px-4 py-3 last:border-b-0">
        <form
          ...
          className="flex flex-wrap items-end gap-3 text-body"
          onSubmit={(event) => {
  ...
  return (
    ...
    >
      <span className="text-xms-ink text-body font-medium">{bucket.label}</span>
      <span className="xms-mono text-xms-label text-body">{bucket.key}</span>
...
function BucketsList({ accountId, canEdit }: { accountId: string; canEdit: boolean }) {
  ...
  const buckets = data ?? [];
  if (buckets.length === 0)
    return <p className="text-xms-label px-4 py-3 text-body">This account has no non-ticket buckets.</p>;
  return (
  ...
  return (
    ...
    >
      {canRead ? (
        ...
        <p className="text-xms-label px-4 py-3 text-body">
