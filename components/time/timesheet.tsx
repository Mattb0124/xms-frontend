  ...
  return (
    <section className="xms-card flex min-w-0 flex-col" data-testid="timesheet" aria-label="Timesheet">
      ...
      <header className="border-xms-line flex min-h-[48px] flex-wrap items-center gap-[14px] border-b px-5 py-3">
        <span className="text-xms-ink text-lead leading-[1.3] font-semibold">Timesheet</span>
        <span className="xms-mono text-xms-muted -ml-[6px] text-body">
          {week.from} to {week.to}
        </span>
        <span className="ml-auto flex items-center gap-4 text-body">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-body">
          <thead className="bg-xms-card sticky top-0 z-10">
            <tr className="border-xms-line-head text-xms-ink border-b text-left text-body font-semibold">
  ...
  return (
    <>
      ...
      <tr className="border-xms-line-row bg-xms-quiet-bg border-b" data-day={day.date} data-tone={tone}>
        <td className="text-xms-ink px-[14px] py-[9px] text-body font-semibold" colSpan={3}>
          {label} <span className="xms-mono text-xms-label font-normal">{day.date}</span>
        </td>
        <td className="xms-mono text-xms-ink px-[14px] py-[9px] text-right text-body font-semibold" data-day-total>
          {day.logged_minutes > 0 ? formatMinutes(day.logged_minutes) : ""}
        </td>
        <td />
        <td className="px-[14px] py-[9px] text-body" data-day-status>
      ...
      {day.entries.map((entry) => {
        ...
        return (
          <tr key={entry.id} className="border-xms-line-row hover:bg-xms-row-hover border-b" data-entry={entry.id}>
            <td className="xms-mono text-xms-label px-[14px] py-[13px] text-right text-body" data-start>
              {start ?? ""}
            </td>
