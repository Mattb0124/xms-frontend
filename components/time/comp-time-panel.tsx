...
export function CompTimePanel({ accountId, today = localToday() }: { accountId: string; today?: string }) {
  ...
  return (
    <Panel
      ...
      actions={
        <div className="flex items-center gap-2 text-body" role="group" aria-label="Comp time range">
    ...
    >
      ...
      {isError ? <p className="text-xms-muted text-body">The comp-time report could not be loaded.</p> : null}
      {data ? (
        <table className="w-full border-collapse text-body" aria-label="Comp time by person">
          <thead>
