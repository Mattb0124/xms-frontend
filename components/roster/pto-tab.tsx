  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      <Panel title="PTO" caption="Time off reduces the month's capacity the moment it is entered">
        ...
        {isError ? <p className="text-xms-muted text-body">The time off could not be loaded.</p> : null}
        {data && data.length === 0 ? <p className="text-xms-label text-body">No time off recorded.</p> : null}
        {data ? (
          <ul className="divide-xms-line divide-y" aria-label="Time off">
            {data.map((pto) => (
              <li key={pto.id} className="flex flex-wrap items-center gap-3 py-2 text-body" data-pto={pto.id}>
                ...
                <span className="text-xms-label text-body" data-fraction>
                  {fractionLabel(pto.fraction)}
                </span>
                {pto.note ? <span className="text-xms-label text-body">{pto.note}</span> : null}
