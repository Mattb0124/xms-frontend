    ...
    >
      ...
      >
        <fieldset className="border-xms-line rounded-control border p-3" disabled={disabled}>
          ...
          <div className="flex flex-wrap gap-3">
            {ISO_WEEKDAYS.map((day) => (
              <label key={day.value} className="flex items-center gap-2 text-body">
                <input
                  type="checkbox"
                  value={day.value}
                ...
                <span className="xms-mono text-xms-muted text-body">{day.value}</span>
              </label>
            ))}
          </div>
        </fieldset>
        ...
        <FieldRow label="Hours per day">
          <span className="xms-mono text-xms-ink text-body" data-testid="hours-per-day">
        ...
        {canEdit ? (
          ...
          <p className="text-xms-label text-body">Needs capacity:manage to change.</p>
        )}
      </form>
    </Panel>
  );
