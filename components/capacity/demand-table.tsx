...
export function DemandTable({ list, canManage, onRemove, removing }: DemandTableProps) {
  ...
  return (
    <Panel
      ...
      flush
    >
      <table className="w-full border-collapse text-body" aria-label="Demand lines">
          ...
          {list.rows.map((row) => (
            <tr key={row.id} className="border-xms-line hover:bg-xms-row-hover h-[40px] border-b" data-demand={row.id}>
              ...
              <td className={cn(CELL, row.account_key && "xms-mono")} data-subject>
                {demandSubject(row)}
                {row.prospect_name ? <span className="text-xms-label ml-2 text-body">prospect</span> : null}
              </td>
              ...
              <td className={cn(CELL, "text-xms-body max-w-[280px] truncate text-body")} title={row.note} data-note>
                {row.note}
              </td>
              {canManage ? (
        ...
        <tfoot>
          <tr className="text-xms-ink text-body font-semibold" data-testid="demand-totals">
