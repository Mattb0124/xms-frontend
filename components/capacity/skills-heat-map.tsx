...
const HEAD = "text-xms-ink px-2 py-2 text-body font-semibold whitespace-nowrap";
...
export function SkillsHeatMap({ matrix, role }: SkillsHeatMapProps) {
  ...
  return (
    <Panel
      ...
      flush
    >
      <table className="w-full border-collapse text-body" aria-label="Skills heat map">
            ...
            <tr key={person.id} className="border-xms-line hover:bg-xms-row-hover border-b" data-person={person.id}>
              <td className="text-xms-ink px-2 py-[9px] align-middle whitespace-nowrap">
                <span className="xms-stack">
                  ...
                  <span className="text-xms-label text-body">{roleLabel(person.role)}</span>
                </span>
              </td>
              {groups.map((group) =>
                group.skills.map((skill, index) => {
                  ...
                  return (
                    <td
                      key={skill.id}
                      className={cn(
                        "xms-mono w-[56px] min-w-[56px] text-center align-middle text-body font-semibold",
