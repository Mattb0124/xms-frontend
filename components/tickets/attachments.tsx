...
export function ScanChip({ state, portal }: { state: ScanState | UploadStage; portal?: boolean }) {
  ...
  if (chip.danger) {
    return (
      <span
        data-scan={state}
        className="inline-flex h-[20px] items-center rounded-pill border border-[color:var(--state-overdue-border)] bg-[color:var(--state-overdue-bg)] px-2 text-body text-[color:var(--state-overdue-text)]"
      >
        {chip.label}
      </span>
    );
  }
  return <StatePill state={chip.ramp} label={chip.label} className="text-body" />;
}
...
export function DropZone({
  ...
  return (
    <div
      ...
      className={cn(
        "border-xms-line text-xms-label flex items-center gap-2 rounded-card border border-dashed px-3 py-2 text-body",
    ...
    >
      ...
      <button
        ...
        className={cn("xms-link ml-auto", portal && "text-body")}
      >
...
export function UploadList({
  ...
  return (
    <ul className="flex flex-col gap-1" aria-label="Uploads">
      {items.map((item) => (
        <li key={item.id} data-stage={item.stage} className="flex flex-wrap items-center gap-2 text-body">
...
export function ScanAcknowledgement({ checked, onChange }: { checked: boolean; onChange: (next: boolean) => void }) {
  return (
    <label className="text-xms-label flex items-center gap-2 text-body">
...
function VisibilityChip({ visibility }: { visibility: AttachmentVisibility }) {
  return (
    <span
      className={cn(
        "rounded-pill px-2 py-[1px] text-body",
...
export function AttachmentRow({
  ...
  return (
    <li data-attachment={attachment.id} data-scan={attachment.scan_state} className="flex flex-col gap-1 py-2">
      ...
      {quarantined ? <p className="text-xms-label text-body">{QUARANTINE_PLACEHOLDER}</p> : null}
      {!portal ? (
        <p className="text-xms-label text-body">by {attachment.uploaded_by_name || attachment.uploaded_by}</p>
      ) : null}
    </li>
  );
}
