...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="flex flex-col gap-[7px]">
      <span className="text-xms-ink text-body leading-[1.4] font-semibold">{label}</span>
      <span className="text-xms-body text-body leading-[1.5] whitespace-pre-wrap">{children}</span>
    </div>
  );
}
      ...
      {linked.length > 0 ? (
        <ul aria-label="Resolution records">
          {linked.map((row) => (
            <li
              key={row.id}
              className="border-xms-line-row flex items-center gap-3 border-b py-[13px] text-body last:border-b-0"
            >
              ...
              <span className="xms-mono text-xms-muted shrink-0 text-body">
