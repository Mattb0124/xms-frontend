        ...
        {rows.map((row) => (
          <li key={row.id} data-participant={row.id} data-status={row.status} className="flex flex-col gap-1 py-2">
            ...
            {row.status === "invited" && row.invited_by_name ? (
              <p className="text-xms-label text-body">asked by {row.invited_by_name}</p>
            ) : null}
            {row.decline_reason ? <p className="text-xms-body text-body">{row.decline_reason}</p> : null}
          </li>
        ))}
      </ul>
