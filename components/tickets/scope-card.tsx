...
const LABEL = "text-xms-label text-body";

function Line({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-[2px]">
      <span className={LABEL}>{label}</span>
      <span className="text-xms-body text-body whitespace-pre-wrap">{children}</span>
    </div>
  );
}
  ...
  return (
    <RailCard caption="Scope">
      <div className="flex flex-col gap-3" data-testid="ticket-scope">
        ...
        {!canWork && !canDecide ? (
          <p className="text-xms-label text-body">Flagging and deciding scope are not yours to do on this ticket.</p>
        ) : null}
