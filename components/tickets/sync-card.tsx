...
function ConflictNote({ link, inline }: { link: TicketSyncLink; inline?: boolean }) {
  ...
  return (
    <p
      ...
      className={
        inline
          ? "mt-2 text-body leading-[1.6]"
          : "rounded-control border border-[color:var(--state-needs-input-border)] bg-[color:var(--state-needs-input-bg)] px-2 py-1 text-body text-[color:var(--state-needs-input-text)]"
      }
...
function OutboundState({ link }: { link: TicketSyncLink }) {
  ...
  return (
    <div className="flex flex-col gap-1" data-outbound-pending={outbound.pending}>
      ...
      <p className={outbound.pending > 0 ? "text-xms-body text-body" : "text-xms-label text-body"}>
        {pendingLabel(outbound.pending)}
        {outbound.failed > 0 ? `, ${outbound.failed} failed` : ""}
      </p>
      {outbound.last_error ? (
        <p className="text-body text-[color:var(--state-overdue-text)]" data-outbound-error>
...
function ExternalRecordLink({ link }: { link: TicketSyncLink }) {
  ...
  if (href === null) {
    return (
      <span className="xms-mono text-xms-body text-body font-medium" data-external={link.external_number}>
...
function LinkFacts({ link }: { link: TicketSyncLink }) {
  ...
  return (
    <div className="border-xms-line bg-xms-quiet-bg xms-mono text-xms-body rounded-card border p-4 text-body leading-[1.9]">
      <p>
        {"external record: "}
        ...
        {links.map((link) => {
          ...
          return (
            <div key={`${link.instance_name}:${link.external_sys_id}`} className="flex flex-col gap-3">
              <div className="xms-note p-4 text-body leading-[1.6]">
                <p>
        ...
        {runs.length > 0 ? (
          <ul className="flex flex-col" aria-label="Recent runs">
            {runs.slice(0, 5).map((run) => (
              <li
                key={run.id}
                className="border-xms-line-row flex items-center gap-3 border-b py-[10px] text-body last:border-b-0"
              >
    ...
    <div className="flex flex-col gap-3">
      {links.map((link) => {
        ...
        return (
          ...
          >
            ...
            {notice ? (
              <p className="text-xms-body text-body" data-notice>
                {notice}
              </p>
            ) : null}
            <OutboundState link={link} />
      ...
      {runs.length > 0 ? (
        <ul className="border-xms-line flex flex-col gap-1 border-t pt-2" aria-label="Recent runs">
          {runs.slice(0, 5).map((run) => (
            <li key={run.id} className="flex items-center gap-2 text-body">
...
export function SyncCard({ ticketId, flush }: { ticketId: string; flush?: boolean }) {
  ...
  if (!data || data.links.length === 0)
    return flush ? <p className="text-xms-label text-body">This ticket is not linked to an external record.</p> : null;
