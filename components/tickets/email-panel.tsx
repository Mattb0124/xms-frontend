...
function LoopChips({ signals }: { signals: string[] }) {
  ...
  return (
    <span className="flex flex-wrap gap-1">
      {signals.map((signal) => (
        <span key={signal} className="bg-xms-tint text-xms-label rounded-pill px-2 py-[1px] text-body">
          {signal.replace(/_/g, " ")}
        </span>
      ))}
    </span>
...
export function InboundRow({ row, onViewRaw }: { row: InboundMessage; onViewRaw: (row: InboundMessage) => void }) {
  return (
    <li className="flex flex-col gap-1 py-2 text-body" data-inbound={row.id} data-disposition={row.disposition}>
      <div className="flex flex-wrap items-center gap-2">
        <span className="xms-mono text-xms-label w-[110px] shrink-0 text-body">{formatStamp(row.received_at)}</span>
        <span className="text-xms-ink font-medium">{row.from_name || row.from_address}</span>
        <span className="xms-mono text-xms-label text-body">{row.from_address}</span>
      ...
      <div className="text-xms-label flex flex-wrap items-center gap-2 text-body">
...
export function OutboundRow({ row }: { row: OutboundMessage }) {
  return (
    <li className="flex flex-wrap items-center gap-2 py-2 text-body" data-outbound={row.id}>
      <span className="xms-mono text-xms-label w-[110px] shrink-0 text-body">{formatStamp(row.created_at)}</span>
      <span className="text-xms-ink font-medium">{kindCopy(row.kind)}</span>
      <span className="text-xms-label">to</span>
      <span className="xms-mono text-xms-label text-body">{row.to_addresses.join(", ")}</span>
  ...
  return (
    <div className="flex flex-col gap-4">
      <section aria-label="Inbound email">
        ...
        {inbound.length === 0 ? <p className="text-xms-label text-body">No email received on this ticket.</p> : null}
      ...
      <section aria-label="Outbound email">
        ...
        {outbound.length === 0 ? <p className="text-xms-label text-body">Nothing sent yet.</p> : null}
        <ul className="divide-xms-line divide-y">
          {outbound.map((row) => (
