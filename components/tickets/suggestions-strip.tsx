...
const REQUEST_BUTTON =
  "border-xms-ai-border text-xms-body hover:bg-xms-card h-[28px] rounded-control border px-3 text-body font-medium disabled:opacity-50";
  ...
  return (
    <section aria-label="Axel suggests" className="flex flex-col gap-2" data-testid="axel-suggest-strip">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {withheld ? (
          <span role="status" className="text-xms-label text-body">
            {withheld}
          </span>
        ) : null}
        {effectiveOff && !withheld ? (
          <span className="text-xms-label text-body">{switchOffTooltip(offCause)}</span>
        ) : null}
      </div>
      ...
      {!open.isLoading && cards.length === 0 && canRequest && !withheld ? (
        <p className="text-xms-label text-body">Nothing suggested yet. Ask for a summary or a draft reply.</p>
      ) : null}
    </section>
  );
}
