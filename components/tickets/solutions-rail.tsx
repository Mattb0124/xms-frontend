        ...
        <div className="flex flex-col gap-3">
          {data.linked.length > 0 ? (
            <ul className="flex flex-col gap-1" aria-label="Resolution records">
              {data.linked.map((row) => (
                <li key={row.id} className="flex flex-col text-body" data-outcome={row.outcome}>
            ...
            {data.articles.length === 0 ? (
              ...
              <ul className="flex flex-col gap-2" aria-label="Matching articles">
                {data.articles.map((hit) => (
                  <li key={hit.id} className="flex flex-col gap-1 text-body">
                    <span className="flex items-center gap-2">
                    ...
                    <span className="flex gap-2">
                      {!readOnly && canResolve ? (
                        <button
                          ...
                          className={`${PRIMARY_BUTTON} h-[26px] px-2 text-body`}
                        >
                          Use this
                        </button>
                      ) : null}
                      <Link
                        ...
                        className={`${SECONDARY_BUTTON} inline-flex h-[26px] items-center px-2 text-body`}
                      >
                        Open
                      </Link>
                    </span>
