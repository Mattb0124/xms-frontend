      ...
      <ol className="flex flex-col gap-[6px]">
        {data.map((row) => (
          <li key={row.id} className="flex flex-col">
            <span className="text-xms-ink text-body leading-[1.35]">{scopeEventLine(row)}</span>
            <span className="text-xms-label xms-mono text-body">{formatMoment(row.at)}</span>
            {row.note ? <span className="text-xms-body text-body leading-[1.35]">{row.note}</span> : null}
          </li>
        ))}
      </ol>
    </div>
