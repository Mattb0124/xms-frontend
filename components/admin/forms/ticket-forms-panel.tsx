...
function PublishedFields({ fields }: { fields: FormField[] }) {
  if (fields.length === 0) return <p className="text-xms-label text-body">This version asks nothing.</p>;
  return (
    <ol className="flex flex-col gap-1" aria-label="Published fields">
      {fields.map((field) => (
        <li key={field.key} className="text-xms-body flex flex-wrap items-baseline gap-2 text-body">
          <span className="text-xms-ink font-medium">{field.label}</span>
          <span className="text-xms-label font-mono text-body">{field.key}</span>
          <span className="text-xms-label text-body">{kindLabel(field.kind)}</span>
          {field.required ? <span className="text-xms-label text-body">required</span> : null}
          <span className="text-xms-label font-mono text-body">{field.maps_to}</span>
          {field.visible_when ? (
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        {fields.length === 0 ? (
          <p className="text-xms-label text-body">This draft asks nothing yet. Add the first field.</p>
        ) : null}

        {problems.length > 0 ? (
          <ul className="mt-3 text-body text-[color:var(--state-overdue-text)]" aria-label="Draft problems">
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <ul className="mb-4 flex flex-col gap-1" aria-label="Forms">
            ...
            return (
              <li key={form.id}>
                <button
                  ...
                  className={cn(
                    "hover:bg-xms-control-hover flex w-full items-center gap-3 rounded-control px-3 py-2 text-left text-body",
          ...
          {forms.length === 0 ? (
            <li className="text-xms-label text-body">
