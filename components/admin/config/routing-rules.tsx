...
function RoutingRulesEditor({ accountId, canWrite }: { accountId: string; canWrite: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <table className="w-full border-collapse text-body" aria-label="Routing defaults">
        <thead>
      ...
      {problems.length > 0 ? (
        <ul className="text-body text-[color:var(--state-overdue-text)]">
      ...
      {canWrite ? (
        ...
        <p className="text-xms-label text-body">Changing the defaults needs the admin:config permission.</p>
      )}
    </div>
  );
}
