    ...
    <div className="flex flex-col gap-3">
      <label className="flex flex-col gap-1 text-body">
        ...
        <textarea
          aria-label="Body"
          spellCheck={false}
          className={cn(INPUT, "xms-mono h-[420px] py-2 text-body leading-[1.5]")}
          value={text}
      ...
      {problems ? (
        <div role="alert" className="text-body text-[color:var(--state-overdue-text)]" data-config-problems>
  ...
  return (
    <Panel title="Versions" caption="Override history, newest first, and the operator default underneath">
      {view.overrides.length === 0 ? (
        <p className="text-xms-label text-body">No override yet. The operator default applies.</p>
      ) : null}
      <ul className="flex flex-col gap-2" aria-label="Override versions">
        {view.overrides.map((row) => (
          <li key={row.id} className="flex items-center gap-2 text-body" data-override-version={row.version}>
      ...
      <div className="border-xms-line mt-3 border-t pt-3">
        {view.default ? (
          <div className="flex items-center gap-2 text-body">
            ...
            <span className="xms-mono text-xms-label ml-auto text-body">{formatDate(view.default.activated_at)}</span>
          </div>
        ) : (
          <p className="text-xms-label text-body">No operator default is active. Run the seed.</p>
        )}
        {view.effective?.source === "override" && view.default ? (
          <details className="mt-2">
            <summary className="text-xms-accent cursor-pointer text-body">Show the operator default body</summary>
            <pre className="xms-mono text-xms-body bg-xms-tint mt-2 max-h-[320px] overflow-auto rounded-control p-3 text-body whitespace-pre-wrap">
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        {data && !data.effective ? (
          <p role="status" className="text-xms-label mb-3 text-body" data-nothing-active>
