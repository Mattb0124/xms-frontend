  ...
  if (state === "disabled_by_loop_guard") {
    return (
      <span
        ...
        className="inline-flex h-[22px] items-center rounded-pill border border-[color:var(--state-overdue-border)] bg-[color:var(--state-overdue-bg)] px-2 text-body text-[color:var(--state-overdue-text)]"
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      <Panel title="Inbound aliases" caption="Addresses that route email to this account">
        ...
        {data && data.length === 0 ? (
          <p className="text-xms-label text-body">No aliases yet. Add the client-facing address below.</p>
        ) : null}
        <ul className="divide-xms-line divide-y" aria-label="Aliases">
          {(data ?? []).map((alias) => (
            <li key={alias.id} className="flex flex-wrap items-center gap-3 py-2 text-body" data-alias={alias.address}>
              <span className="xms-mono text-xms-ink">{alias.address}</span>
              <span className="bg-xms-tint text-xms-label rounded-pill px-2 py-[1px] text-body">{alias.kind}</span>
