 ...
 */
export const INPUT =
  "xms-field border-xms-line bg-xms-card text-xms-ink h-[34px] w-full max-w-[420px] rounded-control border px-2 text-body outline-none disabled:opacity-60";
...
export function ConfirmButton({
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      {armed ? <span className="text-xms-label text-body">Click again to confirm</span> : null}
    </span>
  );
}
...
export function RecordBar({
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-3">
      ...
      <h1 className="text-xms-ink text-lead font-semibold">{title}</h1>
      {pill}
  ...
  return (
    ...
    >
      <input
        ...
        className="h-4 w-4"
      />
      <span className="text-xms-ink text-body">{label}</span>
      {detail ? <span className="text-xms-label ml-auto text-body">{detail}</span> : null}
    </label>
  );
}
...
export function FieldRow({ label, htmlFor, children }: { label: string; htmlFor?: string; children: ReactNode }) {
  return (
    <div className="grid grid-cols-[140px_1fr] items-center gap-3">
      <label htmlFor={htmlFor} className="text-xms-label text-body">
        {label}
      </label>
      {children}
    </div>
