...
export function EngagementsPanel({ accountId }: { accountId: string }) {
  ...
  if (!canRead) {
    return (
      <Panel title="Engagements" caption="Needs the contracts:view permission">
        <p className="text-xms-label text-body">You can see this account but not its engagements.</p>
      </Panel>
    );
  }
  ...
  const columns: DenseColumn<Engagement>[] = [
    { key: "name", title: "Name", sortValue: (row) => row.name },
    {
      ...
      render: (row) => (
        <span className={cn(row.owner_user_id && !owners[row.owner_user_id] ? "xms-mono text-body" : undefined)}>
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span className="text-xms-body text-body" data-notice={row.id}>
            {noticeLabel(row.notice_period_days)}
            {deadline ? <span className="xms-mono text-xms-label ml-2 text-body">decide by {deadline}</span> : null}
          </span>
        );
      },
    },
    ...
    },
    {
      ...
      render: (row) => (
        <span className="text-xms-body text-body" data-alerts={row.id}>
