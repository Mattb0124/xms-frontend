...
describe("batch summary", () => {
  it("renders the counts with their tones, the properties and the log", () => {
    ...
    // Every scorecard number is ink (renders 08 and 10). The tile carried a
    // tone, so a count of zero errors was drawn in green and a count of zero
    // unmatched in amber: a signal where there was none. The counts are still
    // here, and the signal is on the rows they lead to.
    const tile = (label: string) => screen.getByText(label).closest("div") as HTMLElement;
    expect(tile("Unmatched")).toHaveTextContent("Unmatched");
    expect(tile("Errors")).toHaveTextContent("Errors");
    expect(document.querySelector("[aria-label='Counts'] [data-tone]")).toBeNull();
