...
export function RunProgress({ batch }: { batch: BatchDetail }) {
  ...
  return (
    <div role="status" className="bg-xms-tint text-xms-body flex items-center gap-3 rounded-card px-4 py-2 text-body">
      ...
      <span className="text-xms-label ml-auto text-body">refreshes every 10 seconds</span>
    </div>
  );
}
...
export function BatchProperties({ batch, accountName }: { batch: BatchDetail; accountName?: string }) {
  ...
  const rows: { label: string; value: ReactNode; mono?: boolean }[] = [
    ...
    { label: "Mode", value: <DryRunPill dryRun={batch.dry_run} /> },
    {
      label: "Maps",
      value: (
        <span className="xms-mono text-body">
      ...
      <dl className="grid grid-cols-[120px_1fr] gap-x-3 gap-y-2 text-body">
        {rows.map((row) => (
          <div key={row.label} className="contents">
            <dt className="text-xms-label text-body">{row.label}</dt>
            <dd className={row.mono ? "xms-mono text-xms-ink" : "text-xms-ink"}>{row.value}</dd>
          </div>
        ))}
      </dl>
      {batch.error ? (
        <p role="alert" className="mt-3 text-body text-[color:var(--state-overdue-text)]" data-batch-error>
          {batch.error}
        </p>
      ) : null}
    </Panel>
...
export function LogTab({ batch }: { batch: BatchDetail }) {
  return (
    <Panel title="Log" caption={`${batch.log.length} lines`}>
      ...
      {batch.error ? (
        <p className="mt-3 text-body text-[color:var(--state-overdue-text)]">Failed: {batch.error}</p>
      ) : null}
    </Panel>
  );
}
