...
function AuditConditionBuilder({ rows, onChange }: { rows: AuditRow[]; onChange: (rows: AuditRow[]) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2" role="group" aria-label="Conditions">
      ...
      <button type="button" onClick={add} className="text-xms-accent self-start text-body hover:underline">
        + Add condition
      </button>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <section>
        ...
        <dl className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-1 text-body">
        ...
        <section className="grid grid-cols-2 gap-3">
          {(["old", "new"] as const).map((side) => (
            <div key={side}>
              ...
              <pre
                className="xms-mono bg-xms-tint overflow-auto rounded-control p-2 text-body"
      ...
      <section>
        ...
        <pre className="xms-mono bg-xms-tint overflow-auto rounded-control p-2 text-body" data-testid="attrs">
          ...
          footer={
            items.length === 0 ? null : (
              <div className="border-xms-line flex items-center gap-4 border-t px-4 py-2">
                <span className="xms-mono text-xms-label text-body">{summary}</span>
                {cursor ? (
                  <button
                    type="button"
