  ...
  return (
    ...
    >
      ...
      {data && active.length === 0 && retired.length > 0 ? (
        <p className="text-xms-label text-body">Every calendar is retired; clocks run on the wall clock.</p>
      ) : null}
      <ul className="divide-xms-line divide-y" aria-label="Calendars">
        {[...active, ...retired].map((calendar) => (
          ...
          >
            ...
            <span className="xms-mono text-xms-label text-body">{calendar.time_zone}</span>
            <span className="text-xms-label text-body">{formatDuration(weeklyMinutes(calendar.hours))} per week</span>
            {calendar.holidays.length > 0 ? (
              <span className="text-xms-label text-body">{calendar.holidays.length} holidays</span>
            ) : null}
