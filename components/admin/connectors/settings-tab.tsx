    ...
    <Panel title="Settings" caption={`version ${instance.version}`}>
      ...
      >
        ...
        <fieldset className="border-xms-line flex flex-col gap-2 rounded-control border p-3">
          <legend className="text-xms-label px-1 text-body">Automatic trip threshold</legend>
...
export function WatermarkPanel({ instance }: { instance: ConnectorInstance }) {
  ...
  return (
    <Panel title="Inbound watermark" caption="Rewind after a client outage; preview shows how many records come back">
      <p className="xms-mono text-xms-ink mb-3 text-body">Current: {formatDate(instance.inbound_watermark)}</p>
      <form
        className="flex flex-col gap-3"
      ...
      >
        ...
        {previewed ? (
          <p className="text-xms-body text-body" data-preview={preview.records}>
