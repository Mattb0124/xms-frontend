  ...
  if (!permitted) {
    return (
      <Panel title="Connectors" caption="Needs the admin:connectors permission">
        <p className="text-xms-label text-body">You can see this account but not its connectors.</p>
      </Panel>
    );
  }
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_400px]">
      <Panel title="Connector instances" caption="Each instance has its own credential, maps, mode and health">
        ...
        {data && data.length === 0 ? (
          <p className="text-xms-label text-body">No connector on this account yet.</p>
        ) : null}
        <ul className="divide-xms-line divide-y" aria-label="Connector instances">
          {(data ?? []).map((instance) => (
            ...
            >
              ...
              <span className="xms-mono text-xms-label text-body">{instance.table_name}</span>
