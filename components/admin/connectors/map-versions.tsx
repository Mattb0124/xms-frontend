...
export function MapVersions<E>({
  ...
  if (ordered.length === 0) return <p className="text-xms-label text-body">No versions yet. Create a draft.</p>;
  return (
        ...
        return (
          <li key={row.id}>
            <button
              ...
              className={cn(
                "hover:bg-xms-row-hover flex w-full flex-wrap items-center gap-2 px-2 py-2 text-left text-body",
            ...
            >
              ...
              <span className="xms-mono text-xms-muted ml-auto text-body">
                {row.activated_at
...
export function ValidationReportView({ report }: { report: ValidationReport | null }) {
  if (!report) return <p className="text-xms-label text-body">Not validated yet.</p>;
  return (
    <div className="flex flex-col gap-2 text-body" data-report-ok={report.ok ? "true" : "false"}>
      <p
        className={cn(
          "font-medium",
