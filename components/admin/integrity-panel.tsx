...
function Verdict({ tone, children }: { tone: VerdictTone; children: string }) {
  return (
    <span className={cn("text-body", TONE_CLASS[tone])} data-tone={tone}>
      {children}
    </span>
  );
}
...
function Row({ label, detail, right }: { label: string; detail?: string; right?: React.ReactNode }) {
  return (
    <li className="border-xms-line flex flex-wrap items-center gap-3 border-b py-2 text-body last:border-b-0">
      ...
      <div className="flex flex-col gap-4" data-testid="integrity-panel">
          ...
          <Section title="Digest chain">
            ...
            <ul className="flex flex-col">
              {chain.streams.length === 0 ? (
                <li className="text-xms-label py-2 text-body">No stream has been digested yet.</li>
              ) : null}
          ...
          <Section title="Archive">
            <ul className="flex flex-col">
              {archive.streams.length === 0 ? (
                <li className="text-xms-label py-2 text-body">Nothing has been exported to cold storage yet.</li>
              ) : null}
            ...
            <ul className="flex flex-col">
              {streams.map((span) => (
                <Row
                  ...
                  right={<span className="xms-mono text-xms-ink text-body font-semibold">{span.n}</span>}
                />
              ))}
            </ul>
          </Section>
        ...
        {retention ? (
          <Section title="Retention">
            <ul className="text-xms-body flex flex-col gap-1 text-body">
