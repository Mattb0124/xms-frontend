...
describe("billing vocabulary", () => {
  ...
  it("names a period by its month, builds the month's body and shortens the checksum", () => {
    ...
    expect(periodLabel(aBillingPeriod({ starts_on: "2026-09-01", ends_on: "2026-09-15" }))).toBe(
  ...
  it("words the refusals: invalid_transition with the allowed moves, stale_version, period_not_locked", () => {
    ...
    expect(
      describeBillingError(
        billingError({
          status: 409,
          data: { code: "invalid_transition", status: "locked", allowed: ["mark_exported"] },
        }),
      ),
    ...
    expect(
      describeBillingError(
        billingError({
          status: 409,
          data: { code: "invalid_transition", status: "submitted", allowed: ["reopen", "approve", "lock"] },
        }),
      ),
    ...
    expect(
      describeBillingError(billingError({ status: 409, data: { code: "period_not_locked", status: "approved" } })),
    ).toBe("The finance file is produced once the period is locked.");
    expect(
